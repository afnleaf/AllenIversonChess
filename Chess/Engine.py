...
class GameState():
    ...
    def __init__(self):
        # ToDo: Consider bitboard for increased AI speed
        # 8x8 2D list
    ...
    def get_all_possible_moves(self):
        ...
        for row, row_squares in enumerate(self.board):
            for col, square in enumerate(row_squares):
                if bool(square) and (square[0] == 'w') == self.white_to_move:
    ...
    # By setting the direction, enemy, start_row, variables based on the player turn you get where the pawn moves.
    ...
    # Iterate over each direction and check if the corresponding square is a valid move for the knight.
    def get_knight_moves(self, row, col, moves):
        n = len(self.board)
        # list of tuples representing all possible moves for the knight.
    ...
    def get_king_moves(self, row, col, moves):
        ...
        for r in range(row-1, row+2):
            for c in range(col-1, col+2):
                # ignore the square the king is on
                if r == row and c == col:  
                    continue
...
class Move():
    ...
    # returns the form: <start><end> ex e2e4
    def get_chess_notation(self):
        return self.get_rank_file(self.start_row, self.start_col) + self.get_rank_file(self.end_row, self.end_col)

    # returns row and col to letter and number representation of a square ex e2
